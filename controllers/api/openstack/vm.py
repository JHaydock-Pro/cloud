            ...
            try:
                if (imageList[server.image['id']]['aq'] != u'false'):
                    try:
                        ...
                        cherrypy.log('- ' + str(type(e)) + ' when getting Aquilon metadata for VM:' + server.name, username)
                        aquilon = '<i title="Aquilon may still work even if metadata is not stored in OpenStack" style="opacity:.65">Missing Metadata</i>'
